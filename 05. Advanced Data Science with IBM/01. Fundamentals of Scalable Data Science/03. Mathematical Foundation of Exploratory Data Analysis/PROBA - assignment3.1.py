...
def sdTemperature(df,spark):
    ...
    n = float(rddtemp.count())
    sum = rddtemp.sum()
    mean = sum/n
    from math import sqrt
    sd = sqrt(rddtemp.map(lambda x : pow(x-mean,2)).sum()/n)
...
def skewTemperature(df,spark):    
    ...
    from math import sqrt
    sd = sqrt(rddtemp.map(lambda x : pow(x-mean,2)).sum()/n)
    skewness = 1/n * rddtemp.map(lambda x : pow(x-mean,3)/pow(sd,3)).sum()
...
def kurtosisTemperature(df,spark):  
    ...
    sd = sqrt(rddtemp.map(lambda x : pow(x-mean,2)).sum()/n)
    skewness = 1/n * rddtemp.map(lambda x : pow(x-mean,3)/pow(sd,3)).sum()
    kurtosis = rddtemp.map(lambda x : pow(x-mean,4)/pow(sd,4).sum()/n)
...
def correlationTemperatureHardness(df,spark):
    ...
    ntemp = float(rddtemp.count())
    nhard = float(rddhard.count())
    sum_temp = rddtemp.sum()
    sum_hard = rddhard.sum()
    meantemp = sum_temp/ntemp
    meanhard = sum_hard/nhard
    rdd_t_h = rddtemp.zip(rddhard)
    cov_t_h = rdd_t_h.map(lambda (x,y) : (x - meantemp)*(y - meanhard)).sum()/rdd_t_h
    from math import sqrt
